...
# %%
start_and_result = pd.merge(start,result, on = ['Year', 'Grand Prix', 'DriverCode'], how = 'inner')
start_and_result.head()

# %%
start_and_result = start_and_result.dropna(subset = ['Race_Pos'])
# %%
start_and_result['changes'] = start_and_result['Grid_Pos'] - start_and_result['Race_Pos']
...
top10_cars = team_result_changes.sort_values(by='mean', ascending=False).head(10).reset_index()
